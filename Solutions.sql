...
SELECT  ci.city_name, 
		...
		ROUND(SUM(s.total)::numeric/COUNT(DISTINCT s.customer_id)::numeric, 2) as avg_sales_pr_customer
FROM sales s
JOIN customers c
ON s.customer_id = c.customer_id
JOIN city ci
...
SELECT 
	cr.city_name,
	ct.total_revenue as total_sale,
